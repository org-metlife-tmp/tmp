...
<template>
    <el-container id="myExamineApprove">
        <el-header>
            <!--已办数据展示区-->
                ...
                <div class="search-setion">
                    <el-form :inline="true" :model="searchData" size="mini">
                        <el-row>
                            <el-col :span="4">
                                <el-form-item>
                                    <el-input v-model="searchData.init_user_name" clearable
                                              placeholder="请输入发起人"></el-input>
                                </el-form-item>
                            </el-col>
                            <el-col :span="4">
                                <el-form-item>
                                    <el-input v-model="searchData.init_dept_name" clearable
                                              placeholder="请输入部门名称"></el-input>
                                </el-form-item>
                            </el-col>
                            <el-col :span="4">
                                <el-form-item>
                                    <el-input v-model="searchData.init_org_name" clearable
        ...
        <el-main>
            <!--已办数据展示区-->
            <section v-if="!isPending" style="height:100%">
                    ...
                    <el-tab-pane
                        ...
                        <el-table :data="item.tableList"
                            ...
                            <template v-for="head in item.tableHead">
                                ...
                                <!-- 调拨通金额处理千分位 -->
                                <el-table-column
                                        v-else-if="head.prop=='payment_amount' || head.prop== 'total_amount' || head.prop== 'receipts_amount' || head.prop== 'success_amount'"
                                        :key="head.id"
                                        :prop="head.prop"
                                        :label="head.name"
                                ...
                                <!-- 归集通归集额度 -->
                                <el-table-column
                                        v-else-if="head.prop=='collect_type' || head.prop=='allocation_type' || head.prop=='gyl_allocation_type'"
                                        :key="head.id"
                                        :prop="head.prop"
                                        :label="head.name"
                                ...
                                <!-- 归集通归集频率 -->
                                <el-table-column
                                        v-else-if="head.prop=='collect_frequency' || head.prop=='gyl_allocation_frequency' || head.prop=='allocation_frequency'"
                                        :key="head.id"
                                        :prop="head.prop"
                                        :label="head.name"
        ...
        <el-footer>
            ...
            <div class="button-list" v-if="isPending" v-show="activeName!='0'">
                ...
                <el-button type="warning"
                           ...
                           icon="el-icon-circle-check-outline">同意
                </el-button>
            </div>
            <el-pagination
                    background
            ...
            <el-dialog :visible.sync="dialogVisible"
                ...
                <div class="dialogTable">
                    <el-row>
                        <template v-for="detail in currentDetailDialog">
                            <el-col v-if="detail.type==1"
                                    :key="detail.id"
                                    :span="detail.lspan" class="left center">{{detail.label}}
                            </el-col>
                            <el-col class="acc-change" v-else-if="detail.type=='chg'" :key="detail.id">
                                <template v-for="item in detail.list">
                                    <el-col v-if="item.label"
                                            :key="item.id"
                                            :span="item.lspan" class="left">{{item.label}}
                                    </el-col>
                                    <el-col v-else-if="item.parent"
                                            :key="item.id"
                                            :span="item.pspan">{{hasParent(item.prop)}}
                                    </el-col>
                                    <el-col v-else
                                            :key="item.id"
                                            :span="item.pspan">{{dialogData[item.prop]}}
                                    </el-col>
                                </template>
                            </el-col>
                            <el-col v-else-if="detail.label"
                                    :key="detail.id"
                                    :span="detail.lspan" class="left">{{detail.label}}
                            </el-col>
                            <el-col v-else-if="detail.parent"
                                    :key="detail.id"
                                    :span="detail.pspan">{{hasParent(detail.prop)}}
                            </el-col>
                            <el-col v-else-if="detail.prop=='interactive_mode'"
                                    :key="detail.id"
                                    :span="detail.pspan">{{interList[dialogData.interactive_mode]}}
                            </el-col>
                            <el-col v-else-if="detail.prop=='collect_type' || detail.prop=='gyl_allocation_type'"
                                    :key="detail.id"
                                    :span="detail.pspan">{{poolTypeList[dialogData[detail.prop]]}}
                            </el-col>
                            <el-col v-else-if="detail.prop=='collect_frequency' || detail.prop=='gyl_allocation_frequency'"
                                    :key="detail.id"
                                    :span="detail.pspan">{{frequencyList[dialogData[detail.prop]]}}
                            </el-col>
                            <el-col v-else-if="detail.prop=='allocation_type'"
                                    :key="detail.id"
                                    :span="detail.pspan">{{poolTypeList[dialogData.allocation_type]}}
                            </el-col>
                            <el-col v-else-if="detail.prop=='allocation_frequency'"
                                    :key="detail.id"
                                    :span="detail.pspan">{{frequencyList[dialogData.allocation_frequency]}}
                            </el-col>
                            <el-col v-else-if="detail.prop=='deposits_mode'"
                                    :key="detail.id"
                                    :span="detail.pspan">{{depositsList[dialogData.deposits_mode]}}
                            </el-col>
                            <el-col v-else-if="detail.prop=='payment_type'"
                                    :key="detail.id"
                                    :span="detail.pspan">{{dbtTypeList[dialogData.payment_type]}}
                            </el-col>
                            <el-col v-else-if="detail.prop=='service_status'"
                                    :key="detail.id"
                                    :span="detail.pspan">{{billStatusList[dialogData.service_status]}}
                            </el-col>
                            <el-col v-else-if="detail.prop=='pay_mode'"
                                    :key="detail.id"
                                    :span="detail.pspan">{{PayModeList[dialogData.pay_mode]}}
                            </el-col>
                            ...
                            <el-col v-else
                                    :key="detail.id"
                                    :span="detail.pspan">{{dialogData[detail.prop]}}
                            </el-col>
                        </template>
                    </el-row>
                    ...
                    <el-row>
                        ...
                        <el-col :span="enclosurePWidth"><span class="enclosure">{{dialogData.encNumber}}</span>个
                        </el-col>
                    </el-row>
            ...
            <el-dialog title=""
                ...
                <div class="view-mode">
                    <el-button-group>
                        <el-button size="mini" :class="{viewSelect:curInnerTab}" round title="查看批次汇总"
                                   @click="changeTab(true)"><i class="tabBtn total"></i></el-button>
                        <el-button size="mini" :class="{viewSelect:!curInnerTab}" round title="查看批次明细"
                                   @click="changeTab(false)"><i class="tabBtn detail"></i></el-button>
                    </el-button-group>
                </div>
                    ...
                    <ul class="dialog-talbe">
                        ...
                        <li v-if="curBiztype==19">
                            ...
                            <span class="tab-content"
                                  :title="dialogData.recv_account_no">{{dialogData.recv_account_no}}</span>
                        </li>
                        <li v-if="curBiztype==19">
                        ...
                        <li v-if="curBiztype!=19">
                            ...
                            <span class="tab-content"
                                  :title="dialogData.pay_account_no">{{dialogData.pay_account_no}}</span>
                        </li>
                        <li v-if="curBiztype!=19">
                            <span class="table-title">开户行</span>
                ...
                <section id="batchDeatil" v-show="!curInnerTab">
                    <section class="tab-content">
                        <el-table :data="detailTableList"
                                  height="100%"
                                  border size="mini">
                            <el-table-column prop="pay_account_name" v-if="curBiztype==19" label="付款户名"
                                             :show-overflow-tooltip="true"></el-table-column>
                            <el-table-column prop="pay_account_no" v-if="curBiztype==19" label="付款账号"
                                             :show-overflow-tooltip="true"></el-table-column>
                            <el-table-column prop="pay_account_bank" v-if="curBiztype==19" label="付款行"
                                             :show-overflow-tooltip="true"></el-table-column>
                            <el-table-column prop="recv_account_name" v-if="curBiztype!=19" label="收款户名"
                                             :show-overflow-tooltip="true"></el-table-column>
                            <el-table-column prop="recv_account_no" v-if="curBiztype!=19" label="收款账号"
                                             :show-overflow-tooltip="true"></el-table-column>
                            <el-table-column prop="recv_account_bank" v-if="curBiztype!=19" label="收款行"
                                             :show-overflow-tooltip="true"></el-table-column>
            ...
            <el-dialog :visible.sync="lookSonVisible"
                ...
                <el-table :data="sonTableList"
                    ...
                    <el-table-column prop="interactive_mode" label="交互方式"
                                     :show-overflow-tooltip="true"></el-table-column>
                </el-table>
                <el-table :data="sonTableList"
        ...
        created: function () {
            ...
            this.detailDialog = {
                ...
                "32": [
                    ...
                    {id: "12", pspan: 8, prop: "recv_account_no"},
                    {id: "13", lspan: 4, label: "开户行"},
            ...
            return {
                ...
                tableHeadList: {//不同业务种类待办列表要显示的字段
                    ...
                    "32": [
                        ...
                        {id: '9', prop: "recv_account_name", name: '客户名称'},
                        {id: '10', prop: "recv_bank_name", name: '开户名称'},
