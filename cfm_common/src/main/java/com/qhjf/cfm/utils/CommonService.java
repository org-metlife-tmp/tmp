...
public class CommonService {
    private final static Log logger = LogbackLog.getLog(CommonService.class);
...
//                .set("export_count", payRec.get(""));
    ...
    public static Record plfLaBackCheckVoucher(Record payLegalRecord, int iden, String seqnoOrstatmentCode) throws BusinessException {
        ...
        if (inner == 0) {
            description = DateFormatThreadLocal.format("yyMMdd", backDate) + chann.getStr("bankcode") + "第三方结算-批量付款回盘";
        } else if (inner == 1) {
            if (netMode == 0) {
                description = DateFormatThreadLocal.format("yyMMdd", backDate) + chann.getStr("bankcode") + "批量付款回盘-银行净额模式";
            } else if (netMode == 1) {
                description = DateFormatThreadLocal.format("yyMMdd", backDate) + chann.getStr("bankcode") + "批量付款回盘-银行全额模式";
            }
        }
        ...
        return new Record()
                .set("trans_id", 0)
                .set("account_code", subjectCode)
                .set("account_period", DateFormatThreadLocal.format("0MMyyyy", CommonService.getPeriodByCurrentDay(backDate)))
                .set("a_code1", "G")
                .set("a_code2", "CO")
                .set("a_code3", "S")
                .set("a_code5", "SYS")
                ...
                .set("accounting_period", DateFormatThreadLocal.format("yyyy-MM",CommonService.getPeriodByCurrentDay(backDate)))
    ...
    public static Record gmfLaBackCheckVoucher(Record payLegalRecord, int iden, String seqnoOrstatmentCode) throws BusinessException {
        ...
        description = DateFormatThreadLocal.format("yyMMdd", backDate) + acc.getStr("bankcode") + "柜面付费确认";
        transactionReference = "SS" + sftcheck.getStr("code_abbre") + DateFormatThreadLocal.format("YYMM", backDate) + seqnoOrstatmentCode;
        if (iden == 1) {        //LA
            subjectCode = "8360000007";
            debitCredit = "D";
    ...
    public static Record plsLaBackCheckVoucher(Record recvLegalRecord, int iden, String seqnoOrstatmentCode) throws BusinessException {
        ...
        description = DateFormatThreadLocal.format("yyMMdd", backDate) + chann.getStr("bankcode") + "批量收款回盘";
...
//                .set("export_count", payRec.get(""));
    ...
    public static Map<Integer, Date> getPeriod(List<Integer> tradingNo) throws ReqDataException {
        ...
        for (Integer transId : tradingNo) {
            ...
            //获取该月有多少天
            final int day = CommonService.getDayOfMonth(c.get(Calendar.YEAR), c.get(Calendar.MONTH) + 1);
