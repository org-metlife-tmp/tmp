...
public class BatchDbtCallBackJob implements Job{
	
	private static Logger log = LoggerFactory.getLogger(BatchDbtCallBackJob.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		List<Record> executeList = Db.find(Db.getSql("quartzs_job_cfm.get_batch_dbt_nocompletion"), BillStatus.NOCOMPLETION.getKey());
	...
	public class ExecuteThread implements Callable<String> {
		...
		public String call() throws Exception {
			Record record = Db.findFirst(Db.getSql("quartzs_job_cfm.get_batch_dbt_count"), baseInfo.getStr("batchno"),PayStatus.INIT.getKey(),PayStatus.FAILD.getKey(),PayStatus.HANDLE.getKey());
			int count = record.getInt("cnt");
			if(count == 0){
				int flg = Db.update(Db.getSql("quartzs_job_cfm.update_batch_dbt_status"), BillStatus.COMPLETION.getKey(),baseInfo.getLong("id"),baseInfo.getInt("persist_version"));
				if(flg == 0){
