...
import com.qhjf.cfm.web.service.GylSettingService;

import java.util.ArrayList;
import java.util.List;

public class GylSettingController extends CFMBaseController {

    private static final Log log = LogbackLog.getLog(GylSettingController.class);

    private GylSettingService service = new GylSettingService();

    @Auth(hasForces = {"GYLBFJSetting"})
    public void del() {
        try {
    ...
    protected WfRequestObj genWfRequestObj() throws BusinessException {
        ...
        return new WfRequestObj(WebConstant.MajorBizType.GYL, "gyl_allocation_topic", record) {
            @Override
            public <T> T getFieldValue(WebConstant.WfExpressType type) throws WorkflowException {
                ...
                if (type.equals(WebConstant.WfExpressType.AMOUNT)) {
                    return bill_info.get("gyl_allocation_amount");
            ...
            return new ArrayList<WfRequestObj>() {
                {
                    for (final Record rec : wfRequestObjs) {
                        add(new WfRequestObj(WebConstant.MajorBizType.GYL, "gyl_allocation_topic", rec) {
                            @Override
                            public <T> T getFieldValue(WebConstant.WfExpressType type) throws WorkflowException {
                                Record bill_info = getBillRecord();
                                if (type.equals(WebConstant.WfExpressType.AMOUNT)) {
    ...
    private SqlPara getSqlPara(Long[] inst_id, Long[] exclude_inst_id, Record rec) {
        ...
        return Db.getSqlPara("gyl_setting.findGYLPendingList", Kv.by("map", kv));
    }
    ...
    protected List<Record> displayPossibleWf(Record record) throws BusinessException {
        ...
        return CommonService.displayPossibleWf(WebConstant.MajorBizType.GYL.getKey(), getCurUodp().getOrg_id(), biz_id);
    }

    /**
     * 单据详情
     */
    @Auth(hasForces = {"GYLBFJSetting", "MyWFPLAT"})
    public void detail() {
        try {
