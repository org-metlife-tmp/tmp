...
select
	...
	curr_id,
	deposits_mode,
	DATE_FORMAT(create_on,'%Y-%m-%d') as create_on,
	create_by,
	DATE_FORMAT(update_on,'%Y-%m-%d') as update_on,
	update_by,
...
from
	`acc_open_intention_apply_issue` issue join user_info usr on
	usr.usr_id = issue.`user_id`
	and usr.`is_admin` = 0
	and usr.`status` = 1
where
	issue.bill_id = ?
