...
SELECT
  ...
  acc.is_virtual,
  acc.status,
  CONVERT(DATE,afd.create_on,110) AS create_on,
  CONVERT(DATE,afd.apply_on,110) AS apply_on,
...
WHERE 1=1 and afd.delete_flag = 0
    ...
    #for(x : map)
        ...
        AND
          ...
          afd.memo like convert(varchar(5),'%')+convert(varchar(255),#para(x.value))+convert(varchar(5),'%')
          ...
          DATEDIFF(day,#para(x.value),afd.create_on) >= 0
