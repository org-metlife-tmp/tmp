...
import com.qhjf.cfm.web.service.AccFreezeService;
...
public class AccFreezeController extends CFMBaseController {
    private static final Log log = LogbackLog.getLog(AccFreezeController.class);

    private AccFreezeService service = new AccFreezeService();

    @Auth(hasForces = {"AccFreezeAppl"})
    ...
    @Auth(hasForces = {"AccFreezeAppl", "MyWFPLAT"})
    public void detail() {
        Record record = getParamsToRecord();
        try {
    ...
    protected WfRequestObj genWfRequestObj() throws BusinessException {
        ...
        return new WfRequestObj(WebConstant.MajorBizType.ACC_FREEZE_APL, "acc_freeze_and_defreeze_apply", record) {
            @Override
            ...
            return new ArrayList<WfRequestObj>() {
                {
                    for (final Record rec : wfRequestObjs) {
                        add(new WfRequestObj(WebConstant.MajorBizType.ACC_FREEZE_APL, "acc_freeze_and_defreeze_apply", rec) {
                            @Override
                            public <T> T getFieldValue(WebConstant.WfExpressType type) {
                                return null;
                            }
    ...
    private boolean pass(Record rec) {
        ...
        Integer status = WebConstant.AccountStatus.FREEZE.getKey();
    ...
    protected List<Record> displayPossibleWf(Record record) throws BusinessException {
        return CommonService.displayPossibleWf(WebConstant.MajorBizType.ACC_FREEZE_APL.getKey(), getCurUodp().getOrg_id(), null);
    }
