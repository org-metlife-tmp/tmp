...
public class CmbcHisTransQueryInter  implements IMoreResultChannelInter{
	...
	public Map<String, Object> genParamsMap(Record record) {
		Date date = new Date();
		//CMBCTestConfigSection cmbcTestConfigSection = GlobalConfigSection.getInstance().getExtraConfig(IConfigSectionType.BIConfigSectionType.CMBC);
//		int preDay = configSection.getPreDay();
        ...
        /*details.put("BGNDAT", DateKit.toStr(date,"yyyyMMdd"));
        details.put("ENDDAT", DateKit.toStr(date,"yyyyMMdd"));*/
//        String preDate = DateUtil.getSpecifiedDayBefore(date, preDay+1, "yyyyMMdd");
        String preDay = getInter().getChannelConfig("preday");
        Map<String, String> times = TransQueryTimesGenUtil.getInstance().getTransQueryTime(record.getStr("acc_id"), preDay);
        details.put("BGNDAT", times.get("start"));
        details.put("ENDDAT", times.get("end"));
