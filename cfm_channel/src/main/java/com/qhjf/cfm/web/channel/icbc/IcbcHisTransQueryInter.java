...
public class IcbcHisTransQueryInter  implements IMoreResultChannelInter{
	...
	private static final Logger log = LoggerFactory.getLogger(IcbcHisTransQueryInter.class);
	...
	public Map<String, Object> genParamsMap(Record record) {
        ...
        String preDay = inter.getChannelConfig("preday");
	...
	private int parseDirection(JSONObject rd) throws Exception{
		...
		if (null != CASHF) {
			if ("0".equals(CASHF) || "2".equals(CASHF) ||"4".equals(CASHF)) {
				...
				log.warn("工商银行历史交易查询接口，银行返回的交易类型为5（其他）");
			}
		}*/
