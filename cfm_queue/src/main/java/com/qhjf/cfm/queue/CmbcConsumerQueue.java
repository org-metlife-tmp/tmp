 ...
 *
 */
public class CmbcConsumerQueue implements Runnable{
	
	private static Logger log = LoggerFactory.getLogger(CmbcConsumerQueue.class);
	...
	public void run() {
		while(true){
			...
			try {
				queueBean = Queue.getInstance().getCmbcQueue().take();
