...
#sql("getPage")
  SELECT
    ...
    oam.org_name
  FROM
    oa_origin_data oa  left join (
      SELECT oam.oa_org_code, org.name as org_name
      FROM oa_org_mapping oam , organization org
      where oam.tmp_org_code = org.code
    ) oam
    ON oa.apply_org = oam.oa_org_code
  WHERE 1=1
  #if(map != null)
      #for(x : map)
          #if(x.value&&x.value!="")
...
#sql("updProcessStatus")
 update oa_origin_data set process_status = ? ,process_msg = ? where id = ?
#end
