#sql("findHeadPayById")
  select * from oa_head_payment where id = ? and delete_flag = 0
#end
...
#sql("getTodoPage")
  ...
  JOIN oa_origin_data od on od.id = head.ref_id
  WHERE
    head.delete_flag = 0
    and head.service_status in(1,5)
    #if(map != null)
      #for(x : map)
...
#sql("getDonePage")
  ...
  WHERE
    head.delete_flag = 0
    and head.service_status not in(1,5)
  #if(map != null)
      #for(x : map)
...
#sql("findHeadOrgOAPendingList")
  ...
  org.name as org_name,
	...
	cwrei.start_time
FROM
	oa_head_payment ohp,cfm_workflow_run_execute_inst cwrei,oa_origin_data od,organization org
...
#sql("updBillById")
   update oa_head_payment set bank_serial_number = ?,repeat_count = ?,service_status = ?,instruct_code = ?,update_on = ?
