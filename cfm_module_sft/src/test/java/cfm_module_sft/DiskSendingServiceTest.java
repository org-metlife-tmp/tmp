...
public class DiskSendingServiceTest {
	...
	public void findMasterByBatchNoTest() {
		...
		ids.add(240890);ids.add(240890);
		Record record = new Record();
		record.set("id", ids);
		//List<Record> deleteRecords = Db.find(Db.getSqlPara("voucher.delVoucherList", Kv.by("map", record.getColumns())));
//		Db.deleteById("sun_voucher_data","id",ids.toArray());
		int s = Db.update(Db.getSqlPara("voucher.delVoucherList", Kv.by("map", record.getColumns())));
	}
...
//			Record mbRecord = Db.findById("pay_batch_total_master", "id", 2084);
	...
	public void repeatSendbankPayTest(){
		...
		boolean tx = Db.tx(new IAtom() {
			@Override
			public boolean run() throws SQLException {
				total[0] = Db.findFirst("select * from pay_batch_total where id=? and persist_version=?", id, persistVersion);
				if (total[0] == null) {
					errMst[0] = "未找到相应的子批次！";
					return false;
				}
				int update = Db.update("update pay_batch_total set persist_version = ? where id=? and persist_version=?", (persistVersion+1), id, persistVersion);
				if (update != 1) {
