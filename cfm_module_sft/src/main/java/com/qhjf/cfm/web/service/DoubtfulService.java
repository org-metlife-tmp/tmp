 ...
 * @author GJF
 */
public class DoubtfulService {
    private static Logger log = LoggerFactory.getLogger(DoubtfulService.class);
    /**
     * 可疑数据 - 查看分页列表
     *
     * @param pageNum
    ...
    public void reject(final Record record, final UserInfo userInfo) throws BusinessException {
        ...
        if(WebConstant.SftOsSource.LA.getKey() == osSource){
            originTable = "la_origin_pay_data";
            doubtfulTable = "la_check_doubtful";
        }else if(WebConstant.SftOsSource.EBS.getKey() == osSource){
            originTable = "ebs_origin_pay_data";
            doubtfulTable = "ebs_check_doubtful";
        }else{
    ...
    public void pass(final Record record, final UserInfo userInfo) throws BusinessException {
        ...
        legalRecord.set("recv_acc_name", TypeUtils.castToString(doubtfulRecord.get("recv_acc_name")));
        legalRecord.set("recv_cert_type", TypeUtils.castToString(doubtfulRecord.get("recv_cert_type")));
        legalRecord.set("recv_cert_code", TypeUtils.castToString(doubtfulRecord.get("recv_cert_code")));
        ...
        legalRecord.set("create_time", TypeUtils.castToDate(doubtfulRecord.get("create_time")));
        ...
        boolean flag = Db.tx(new IAtom() {
            ...
            public boolean run() throws SQLException {
                ...
                //保存合法数据表
                boolean c = Db.save("pay_legal_data", legalRecord);
                if (!c) return false;
                //获取合法数据表id
                ...
                if(WebConstant.SftOsSource.LA.getKey() == osSource){
                    ...
                    recordExt.set("pay_date", TypeUtils.castToDate(doubtfulRecord.get("pay_date")));
                    recordExt.set("recv_cert_type", TypeUtils.castToString(doubtfulRecord.get("recv_cert_type")));
                    ...
                    boolean d = Db.save("la_pay_legal_data_ext", recordExt);
                    if (!d) return false;
                }else if(WebConstant.SftOsSource.EBS.getKey() == osSource){
                    recordExt.set("legal_id", zid);
                    recordExt.set("origin_id", TypeUtils.castToLong(doubtfulRecord.get("origin_id")));
                    recordExt.set("insure_type", TypeUtils.castToString(doubtfulRecord.get("insure_type")));
                    recordExt.set("org_code", TypeUtils.castToString(doubtfulRecord.get("org_code")));
                    recordExt.set("preinsure_bill_no", TypeUtils.castToString(doubtfulRecord.get("preinsure_bill_no")));
                    recordExt.set("insure_bill_no", TypeUtils.castToString(doubtfulRecord.get("insure_bill_no")));
                    recordExt.set("branch_bill_no", TypeUtils.castToString(doubtfulRecord.get("branch_bill_no")));
                    recordExt.set("biz_type", TypeUtils.castToString(doubtfulRecord.get("biz_type")));
                    recordExt.set("pay_mode", TypeUtils.castToString(doubtfulRecord.get("pay_mode")));
                    recordExt.set("pay_date", TypeUtils.castToDate(doubtfulRecord.get("pay_date")));
                    recordExt.set("recv_cert_type", TypeUtils.castToString(doubtfulRecord.get("recv_cert_type")));
                    ...
                    recordExt.set("op_code", TypeUtils.castToString(doubtfulRecord.get("op_code")));
                    recordExt.set("op_name", TypeUtils.castToString(doubtfulRecord.get("op_name")));
                    boolean d = Db.save("ebs_pay_legal_data_ext", recordExt);
                    if (!d) return false;
                }else{
                    return false;
                }
