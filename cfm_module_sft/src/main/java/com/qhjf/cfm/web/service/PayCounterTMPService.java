 ...
 *
 */
public class PayCounterTMPService {

	private final static Log logger = LogbackLog.getLog(PayCounterTMPService.class);
	...
	public Page<Record> list(int pageNum, int pageSize, Record record, Long org_id) throws UnsupportedEncodingException, BusinessException {
		...
		record.set("service_status", service_status);
		 ...
		 SqlPara sqlPara = Db.getSqlPara("pay_counter.findTMPPayCounterList", Kv.by("map", record.getColumns()));				
	...
	public void supplement(final Record record, UserInfo userInfo) throws ReqDataException, EncryAndDecryException {
		...
		final Long id = record.getLong("pay_id");
		Record recv_counter_match = Db.findById("recv_counter_match", "id",id);
		if(WebConstant.SftLegalData.REVOKE.getKey() == recv_counter_match.getInt("status")
				|| WebConstant.SftLegalData.GROUPBATCH.getKey() == recv_counter_match.getInt("status")) {
			logger.error("====此状态的数据不能进行补录==="+recv_counter_match.getInt("status"));
		...
		if(StringUtils.isBlank(record.getStr("recv_cnaps_code"))) {
			...
			if(null == findFirst) {
				throw new ReqDataException("请重新选择具体的开户行");
			}
		...
		boolean flag = Db.tx(new IAtom() {			
			...
			public boolean run() throws SQLException {
					boolean update = CommonService.update("recv_counter_match", 
						...
						.set("supply_status", WebConstant.Sft_supplyStatus.YBL.getKey()).set("match_recv_acc_no",encry_recv_acc_no)
						.set("match_recv_bank_name", record.get("recv_bank_name")).set("recv_bank_cnaps", record.get("recv_cnaps_code"))
						.set("payment_summary", record.get("payment_summary")).set("persist_version", persist_version+1).set("match_recv_acc_name", record.get("recv_acc_name")), 
						 new Record().set("id", id).set("persist_version", persist_version));
					if(update) {
						// 删除附件  暂时将附件与合法数据id关联 ===提交后将附件与单据关联
						CommonService.delFileRef(WebConstant.MajorBizType.GMF.getKey(), id);
						if (list != null && list.size() > 0) {
							// 保存附件
							return CommonService.saveFileRef(WebConstant.MajorBizType.GMF.getKey(), id, list);
						}	
						return true ;
					}
					return false ;
	 ...
	 * @throws ReqDataException 
	 */
	public void revokeToTMP(final Record record, UserInfo userInfo) throws ReqDataException {
		Long usr_id = userInfo.getUsr_id();
		final Record user_record = Db.findById("user_info", "usr_id",usr_id);
		if(null == user_record){
			throw new ReqDataException("当前登录人未在用户信息表内配置");
		}
		final Long id = TypeUtils.castToLong(record.get("pay_id"));
		// 更新 status 为 已拒绝 . 
		Record recv_counter_match = Db.findById("recv_counter_match", "id",id);
		if(WebConstant.SftLegalData.REVOKE.getKey() == recv_counter_match.getInt("status")
				|| WebConstant.SftLegalData.GROUPBATCH.getKey() == recv_counter_match.getInt("status")) {
			logger.error("====此状态的数据不能进行拒绝==="+recv_counter_match.getInt("status"));
		...
		boolean flag = Db.tx(new IAtom() {
			...
			public boolean run() throws SQLException {
				boolean update = CommonService.update("recv_counter_match",
						...
						new Record().set("id", id).set("persist_version", record.getInt("persist_version")));
				return update;
			}
		});
		if (!flag) {
	...
	public void confirm (Record record, final UserInfo userInfo, UodpInfo uodpInfo) throws ReqDataException{
		...
		List<Record> statuss = Db.find(Db.getSqlPara("pay_counter.findDistinctTMPStatus", Kv.by("map", pay_ids)));
		...
		List<Record> Details = Db.find(Db.getSqlPara("pay_counter.checkBatchTMPDetail", Kv.by("map", pay_ids)));				
        ...
        for (Record rec : Details) {
        	...
        	insertrec.set("legal_id", rec.get("id"))
        	          .set("origin_id", rec.get("id"))
        	          .set("source_sys", 3) //暂定TMP系统枚举为3
        	          .set("org_id", org_id) 
        	          ...
        	          .set("recv_account_no", rec.get("match_recv_acc_no"))
        	          .set("recv_account_name", rec.get("match_recv_acc_name"))
        	          .set("recv_account_cur", "CNY")
        	          .set("recv_bank_prov", cnapsRecord.get("province"))
        	          .set("recv_bank_city", cnapsRecord.get("city"))
        	          .set("recv_bank_name", rec.get("match_recv_bank_name"))
        	          .set("recv_bank_cnaps", rec.get("recv_bank_cnaps"))
        	          .set("recv_bank_type", cnapsRecord.get("bank_type"))
        	          ...
        	          .set("is_match", WebConstant.YesOrNo.YES.getKey());
        	updateLegalRecord.set("id", rec.get("id"))
      ...
      boolean flag = Db.tx(new IAtom() {			
			...
			public boolean run() throws SQLException {
			    ...
			    int[] batchUpdate = Db.batchUpdate("recv_counter_match", "id", updateLegalRecords, updateLegalRecords.size());
			    boolean updateResult = ArrayUtil.checkDbResult(batchUpdate);
			    logger.info("======更新recv_counter_match结果====="+updateResult);
			    if(updateResult) {
