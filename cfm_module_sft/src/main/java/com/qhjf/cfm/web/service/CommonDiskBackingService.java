 ...
 *
 */
public class CommonDiskBackingService {
	...
	public Map<String, Object> validateFile( final UploadFileScaffold ufs,
			Map<String, Object> result, Map<String, String> tablenames) throws IOException, ReqDataException {

		final Record user_record = Db.findById("user_info", "usr_id", tablenames.get("user_id"));
		if (null == user_record) {
			throw new ReqDataException("当前登录人未在用户信息表内配置");
		}

		// 判断此条数据的出盘模板属于哪个通道,判断回盘文件的文件格式
		Record recv_batch_total_master = Db.findById(tablenames.get("main_table"), "id", tablenames.get("main_id"));
		Record recv_batch_total = Db.findById(tablenames.get("son_table"), "id", tablenames.get("son_id"));
		final String child_batchno = recv_batch_total.getStr("child_batchno");
		final Record channel_setting = Db.findById("channel_setting", "id", tablenames.get("channel_id"));
    	...
    	Integer document_moudle = configs_tail.getInt("document_moudle") ;
