 ...
 * @date 2018年9月18日
 */
public class DiskBackingService {
	...
	public Map<String, Object> validateAndWriteToUrl(String user_id, final UploadFileScaffold ufs,
		...
		// 配置文件中的账号 作为回传ebs的付款账号
		final String pay_acc_no = PlfConfigAccnoSection.getInstance().getAccno();
		logger.info("===========配置文件获取到的账号======="+pay_acc_no);
		...
		// 子批次表中存储的报盘总金额
		BigDecimal sendamount = pay_batch_total.getBigDecimal("total_amount");
		// 回盘文件总行数, 
		int backnum = 0;
		// 回盘文件子批次行数
		int sonnum = 0;
...
*/							SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
							...
							// 配置文件中的账号 作为回传ebs的付款账号
							String pay_acc_no = PlfConfigAccnoSection.getInstance().getAccno();
							logger.info("===========配置文件获取到的账号======="+pay_acc_no);
		...
		if (!updateflag) {
			...
			try {
				if(source_sys == 0) {
					logger.info("======回调LA");
					originRecord = Db.find(Db.getSql("disk_backing.selectLaOriginData"), child_batchno);
				}else if(source_sys == 1){
					logger.info("======回调EBS");
					originRecord = Db.find(Db.getSql("disk_backing.selectEbsOriginData"), child_batchno);
				}
