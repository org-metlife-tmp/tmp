 ...
 *
 */
public class RecvCounterCheckService {
	
    private static Logger log = LoggerFactory.getLogger(RecvCounterCheckService.class);
	...
	public Page<Record> list(int pageSize, int pageNum, Record record, UserInfo userInfo, UodpInfo curUodp) {
	    if (record.get("recv_mode")==null || "".equals(TypeUtils.castToString(record.get("recv_mode")))) {
	            record.set("recv_mode", new int[]{
	                    ...
	                    WebConstant.Sft_RecvGroupCounter_Recvmode.ZP.getKey(),
	                    WebConstant.Sft_RecvGroupCounter_Recvmode.WYHK.getKey()
	            });
	    }
	...
	public void confirm(final Record record, final UserInfo userInfo) throws BusinessException {
		...
		//获取勾选的交易和批次
		final List<Record> batchList = Db.find(Db.getSqlPara("recvcountercheck.findBillList", Kv.by("batchNo", batchNo)));
		...
		boolean flag = Db.tx(new IAtom() {
			...
			public boolean run() throws SQLException {
				...
				//存储关系
				int[] result = Db.batchSave("recv_counter_checked", records, 1000);
				if(!ArrayUtil.checkDbResult(result)){
					return false;
				}
				try {
					//生成凭证信息
					log.info("个单团单对账凭证生成开始");
					boolean vouFlag = CheckVoucherService.sunVoucherData(batchNo, tradingNo, batchList, tradList, WebConstant.MajorBizType.GMSTD.getKey(), seqnoOrstatmentCode, userInfo);
					log.info("个单团单对账凭证生成结束");
					return vouFlag;
