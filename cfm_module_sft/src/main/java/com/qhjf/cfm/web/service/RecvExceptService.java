 ...
 * @author GJF
 */
public class RecvExceptService {
    private static Logger log = LoggerFactory.getLogger(RecvExceptService.class);
    /**
     * 异常处理 - 查看分页列表
     *
     * @param pageNum
    ...
    public Page<Record> exceptlist(int pageNum, int pageSize, final Record record, UodpInfo uodpInfo) throws BusinessException {
        ...
        SqlPara sqlPara = Db.getSqlPara("recvexcept.exceptlist", Kv.by("map", record.getColumns()));
    ...
    public Record detail(final Record record, UserInfo userInfo, UodpInfo uodpInfo) throws BusinessException {
        ...
        //根据单据id查询单据信息
        List<Record> records = Db.find(Db.getSql("recvexcept.findbatchtotalbyid"),id);
        Record dbRec = null;
    ...
    public void revoke(final Record record, final UserInfo userInfo,  final UodpInfo curUodp) throws BusinessException {
        ...
        final Record childRecord = Db.findById("recv_batch_total", "id", id);
        if(childRecord == null){
        ...
        boolean flag = Db.tx(new IAtom() {
            ...
            public boolean run() throws SQLException {
                ...
                try {
                    flows = CommonService.displayPossibleWf(WebConstant.MajorBizType.PLS_EXCEPT_BACK.getKey(), curUodp.getOrg_id() ,null);
                } catch (BusinessException e) {
                    e.printStackTrace();
                    log.error("============获取批量收回退审批流异常");
                    return false ;
                }
...
//                childRecord.set("service_status", WebConstant.SftCheckBatchStatus.HTSPZ.getKey());
                //TODO
                WfRequestObj wfRequestObj = new WfRequestObj(WebConstant.MajorBizType.PLS_EXCEPT_BACK, "recv_batch_total", childRecord) {
                    @Override
    ...
    public boolean hookPass(Record record) {
    	...
    	Record position = Db.findFirst(Db.getSql("except.findFirstApprovePosition"), WebConstant.MajorBizType.PLS_EXCEPT_BACK.getKey(),record.getLong("id"));
    	if(position != null){
