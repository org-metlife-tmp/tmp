...
public class CaptchaRender extends Render {
	 ...
	 * md5随机盐值
	 */
	private static final String md5Salt = HashKit.generateSalt(3);
	...
	public CaptchaRender(int imgRandNumber) {
		...
		this.md5RandonCode = HashKit.md5((randonCode+md5Salt).toUpperCase());
	}
	...
	public static boolean validate(Controller controller, String inputRandomCode) {
		if (StrKit.isBlank(inputRandomCode))
			return false;
		LogKit.debug(" source inputRandomCode :"+inputRandomCode +" source md5Salt :"+md5Salt);
		inputRandomCode = HashKit.md5((inputRandomCode+md5Salt).toUpperCase());
		LogKit.debug(" dest inputRandomCode :"+inputRandomCode);
		String md5RandomCode = controller.getCookie(DEFAULT_CAPTCHA_MD5_CODE_KEY);
		LogKit.debug(" dest md5RandomCode :"+md5RandomCode);
