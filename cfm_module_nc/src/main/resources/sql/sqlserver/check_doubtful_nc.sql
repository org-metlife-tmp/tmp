...
SELECT
	...
	ood.flow_id
FROM
	nc_check_doubtful ocd,
	nc_origin_data ood,
	nc_org_mapping oom,
	organization org
WHERE
	ocd.origin_id = ood.id
	AND ood.apply_org = oom.nc_org_code
    ...
    #for(x : map)
          ...
          AND
              ...
              (
              ood.recv_acc_no like convert(varchar(5),'%')+convert(varchar(255),#para(x.value))+convert(varchar(5),'%')
              or
              ood.recv_acc_name like convert(varchar(5),'%')+convert(varchar(255),#para(x.value))+convert(varchar(5),'%')
              )
