...
public class DbtTradService {
    ...
    public Page<Record> billList(int pageNum, int pageSize, final Record record) {
        SqlPara sqlPara = Db.getSqlPara("dbttrad.billList", Kv.by("map", record.getColumns()));
    ...
    public List<Record> tradingList(final Record record) {
        SqlPara sqlPara = Db.getSqlPara("dbttrad.tradingList", Kv.by("map", record.getColumns()));
    ...
    public Page<Record> confirm(final Record record, final UserInfo userInfo) throws BusinessException {
        ...
        Record innerRec = Db.findById("inner_db_payment", "id", billId);
        if (innerRec == null) {
        ...
        boolean flag = Db.tx(new IAtom() {
            ...
            public boolean run() throws SQLException {
                ...
                if (CommonService.update("inner_db_payment", set, where)) {
                    for (Record r : records) {
                        boolean i = Db.save("inner_pay_trans_checked", r);
                        boolean t = CommonService.update("acc_his_transaction",
                                new Record().set("is_checked", 1).set("checked_ref", "inner_db_payment").set("ref_bill_id", billId),
                    ...
                    try {
                        //生成凭证信息
                        CheckVoucherService.sunVoucherData(tradingNo, billId, WebConstant.MajorBizType.INNERDB.getKey(), tradMap, userInfo);
        ...
        if (!flag) {
            ...
            SqlPara sqlPara = Db.getSqlPara("dbttrad.billList", Kv.by("map", rd.getColumns()));
    ...
    public Page<Record> confirmbillList(int pageNum, int pageSize, final Record record) {
        SqlPara sqlPara = Db.getSqlPara("dbttrad.confirmbillList", Kv.by("map", record.getColumns()));
        return Db.paginate(pageNum, pageSize, sqlPara);
    }
    ...
    public List<Record> confirmTradingList(final Record record) {
        return Db.find(Db.getSql("dbttrad.confirmTradingList"), record.getInt("bill_no"));
    }
