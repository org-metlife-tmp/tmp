...
import com.qhjf.cfm.web.webservice.la.queue.noResp.LaNoRespConsumerQueue;
...
private static final String threadName = "LA批收消费者队列";
	...
	public boolean start() {
		...
		LaNoRespConsumerQueue consumerNoResp = new LaNoRespConsumerQueue();
		for(int i = 0;i < 1;i++){
			log.debug("LA批收noResp消费者"+i);
			new Thread(consumerNoResp,threadName+"["+(i+1)+"]").start();
		}
		return true;
	}
