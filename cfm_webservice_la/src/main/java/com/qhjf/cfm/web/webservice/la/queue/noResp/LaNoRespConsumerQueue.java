 ...
 *
 */
public class LaNoRespConsumerQueue implements Runnable{

	private static Logger track = LoggerFactory.getLogger(BatchRecvLogger.class);
	private static Logger log = LoggerFactory.getLogger(LaNoRespConsumerQueue.class);
	...
	public void run() {
		while(true){
			...
			try{
				log.debug("LA pishou queue size = {}", LaNoRespQueue.getInstance().getQueue().size());
				LaNoRespQueueBean queueBean = null;
				queueBean = LaNoRespQueue.getInstance().getQueue().take();
...
//				String status = getStatusSkipEAI(json);
	...
	private void processFail(LaNoRespQueueBean queueBean){
		...
		for(LaRecvCallbackBean bean : beans){
			...
			try{
				Record record = Db.findFirst(Db.getSql("webservice_la_recv_cfm.getRecordByPayCode"), bean.getDdderef());
				if (record.get("la_callback_resp_time")==null){
	...
	private void processSuccess(JSONObject resp, LaNoRespQueueBean queueBean){
		...
		for(int i = 0;i<array.size();i++){
			...
			boolean flag = Db.tx(new IAtom() {
				...
				public boolean run() throws SQLException {
					...
					Record record = Db.findFirst(Db.getSql("webservice_la_recv_cfm.getRecordByPayCode"), payCode);
					if(processStatus.equalsIgnoreCase("SC")&&record.get("la_callback_resp_time")==null){
						Record setRecord = new Record()
